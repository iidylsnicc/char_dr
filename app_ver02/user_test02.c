 ...
 * tac gia : dat.a3cbq11@gmail.com
 * ngay tao: 9/12/2018110
...
#define DEVICE_NODE "/dev/vdy_device_file"
...
/* ham kiem tra entry point write cua vdy driver */
void write_data_chardev(){

    int ret = 0;
    char user_buf[BUFFER_SIZE];
    printf("Enter your messa\n");
    scanf("%[^\n]s", user_buf);
...
/*cac ham kiem tra entry point ioctl cua vdy driver*/
...
void control_read_chardev(){
    ...
    status_t status;
    char c = 'n';
    printf("Do you want to enable reading from data register (y/n)\n");
    scanf("%c", &c);
    if(c== 'y')
        isReadable = 1;
    else if (c == 'n')
...
void control_write_chardev(){
    ...
    status_t status;
    char c = 'n';
    printf("Do you want to enable writing to data register (y/n)\n");
    scanf("%c", &c);
    if(c== 'y')
        isWriteable = 1;
    else if (c == 'n')
...
int main() {
    ...
    printf("\tr (to read data from device node( device file ) )\n");
    printf("\tw (to write data to device node( device file ) )\n");
        ...
        switch (option) {
            ...
            case 'c':
                if (fd > -1)
                    ...
                    printf("%s chua mo nen dell co dong duoc\n", DEVICE_NODE);
                fd = -1;
                break;
            case 'r':
                read_data_chardev();
            ...
            case 'q':
                ...
                return 0;
            default:
                printf("dell co tuy chon %c, chon lai\n", option);
                break;
        }
    };
}